...
namespace lunatic::frontend {
  ...
  namespace detail {
    ...
    inline U decode_move_shifted_register(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_add_sub(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_mov_cmp_add_sub_imm(u16 opcode, T& client) {
      ARMDataOp op;
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_alu(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_high_register_ops(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_relative_pc(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_store_offset_reg(u16 opcode, T& client) {
      auto info = ARMSingleDataTransfer{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_store_signed(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_store_offset_imm(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_store_half(u16 opcode, T& client) {
      auto info = ARMHalfwordSignedTransfer{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_store_relative_sp(u16 opcode, T& client) {
      auto info = ARMSingleDataTransfer{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_load_address(u16 opcode, T& client) {
      auto info = ARMDataProcessing{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_add_sp_offset(u16 opcode, T& client) {
      auto info = ARMDataProcessing{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_push_pop(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_ldm_stm(u16 opcode, T& client) {
      bool writeback = true;
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_conditional_branch(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_svc(u16 opcode, T& client) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_unconditional_branch(u16 opcode, T& client) {
      auto info = ARMBranchRelative{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_branch_link_prefix(u16 opcode, T& client) {
      auto info = ARMDataProcessing{};
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_branch_link_suffix(u16 opcode, T& client, bool exchange) {
    ...
    template<typename T, typename U = typename T::return_type>
    inline U decode_branch_link_full(u32 opcode, T& client) {
  ...
  template<typename T, typename U = typename T::return_type>
  inline U decode_thumb(u32 opcode, T& client) {
