  ...
  class BasicBlockCache {
    public:
     ~BasicBlockCache() {
      ...
      BasicBlock* Get(BasicBlock::Key key) const {
        auto& table = m_data[key.value >> 19];
        if (table == nullptr) {
          return nullptr;
        }
      ...
      void Set(BasicBlock::Key key, BasicBlock* block) {
        ...
        auto& table = m_data[hash0];

        if (table == nullptr) {
          m_data[hash0] = std::make_unique<Table>();
        }
      ...
      std::unique_ptr<Table> m_data[0x40000]{};
  };
